...
public class CommentRestController {
   ...
   // @DeleteMapping 
   @DeleteMapping("/comment/delete/{no}/{room_no}")
   public ResponseEntity<Map> comment_delete(@PathVariable("no") int no,@PathVariable("room_no") int room_no)
   {
	   Map map=new HashMap();
	   try
	   {
		   List<CommentDTO> list=cService.commentDelete(no,room_no);
		   map.put("comments", list);
	   }catch(Exception ex)
	   {
		   return new ResponseEntity<>(null,HttpStatus.INTERNAL_SERVER_ERROR);
